...
public indirect enum KeyType {
    ...
    case string, number, date, anyObject
...
public enum InputValidationError: Error {
    ...
    case referenceErrorInUrl(model: Model.Type)
    ...
    var message: String? {
        ...
        case .referenceErrorInUrl(let model): return "Reference error in URL, entity of type \(model) not found"
...
public func validate(document: Document, requiredFields: [String : KeyType] = [:], optionalFields: [String : KeyType] = [:], errorKeyPrefix: String = "") throws {
    ...
    for (key, value) in document {
        ...
        func expect(_ expectedType: KeyType, for value: Value) throws {
        ...
        try expect(expectedType, for: value)
    }
}
...
public func validateReference<M : Model>(_ id: ObjectId, to: M.Type) throws {
    ...
    guard try collection.count(matching: "_id" == id) == 1 else {
        ...
        throw InputValidationError.referenceErrorInUrl(model: M.self)
    }
}
